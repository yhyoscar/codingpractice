...
def delete_nodes(root, keys, newroots):
    if root is not None:
        if root.val in keys:
            keys.remove(root.val)
            if root.left is not None:
                newroots.append(root.left)
            if root.right is not None:
                newroots.append(root.right)
            root.left = delete_nodes(root.left, keys, newroots)
            root.right = delete_nodes(root.right, keys, newroots)
            root = None
        else:
            root.left = delete_nodes(root.left, keys, newroots)
            root.right = delete_nodes(root.right, keys, newroots)
...
newroots = [root]
delete_nodes(root, [5,1], newroots)

print(len(newroots))

for newroot in newroots:
    if newroot is not None:
        print(codec.serialize(newroot, flag=3))
